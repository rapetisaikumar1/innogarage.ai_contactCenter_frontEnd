      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-slate-800 truncate">
            {actionState === 'done' ? doneMessage : 'Candidate transfer request'}
          </p>
          <p className="text-xs text-slate-500 truncate">
            {actionState === 'done'
              ? 'The request has been updated successfully.'
              : `${current.body} Accept to move this candidate and WhatsApp conversation to your queue.`}
          </p>
        </div>
        ...
        {actionState === 'idle' && (
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600 shadow-sm transition-colors hover:border-slate-300 hover:bg-slate-50 hover:text-slate-800"
            >
              Reject
            </button>
            <button
              onClick={handleAccept}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700"
            >
              Accept
            </button>
          </div>
        )}
        {actionState !== 'idle' && actionState !== 'done' && (
          <div className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm flex-shrink-0 min-w-[8.75rem]">
            <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
            {actionState === 'accepting' ? 'Accepting...' : 'Rejecting...'}
          </div>
        )}
        <button
          onClick={dismiss}
