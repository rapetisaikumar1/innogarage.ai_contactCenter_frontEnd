...
export default function CandidateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !candidate) return <div className="p-8 text-sm text-gray-600">{error ?? 'Candidate not found'}</div>;
  ...
  return (
    <div className="space-y-4">
      {/* Back link */}
      <Link href="/candidates" className="text-sm text-gray-600 hover:underline">← Back to Candidates</Link>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3">
            <select
              ...
              className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-500 disabled:opacity-50"
            >
              {ALL_STATUSES.map((s) => (
            ...
            <Link
              href={`/inbox/${id}`}
              className="px-3 py-1.5 text-sm border border-gray-300 text-gray-700 bg-gray-50 rounded-lg hover:bg-gray-100 flex items-center gap-1.5"
            >
            ...
            <button
              onClick={handleCall}
              disabled={calling}
              className="px-3 py-1.5 text-sm border border-gray-300 text-gray-700 bg-gray-50 rounded-lg hover:bg-gray-100 flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
        ...
        {/* Call feedback toast */}
        {callMessage && (
          <div className={`mt-3 px-4 py-2 rounded-lg text-sm ${callMessage.type === 'success' ? 'bg-gray-50 text-gray-700 border border-gray-200' : 'bg-gray-50 text-gray-700 border border-gray-200'}`}>
            {callMessage.text}
          </div>
        )}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="text-xs px-3 py-1 bg-gray-900 text-white rounded-lg hover:bg-black"
              >
                + Schedule
              </button>
            </div>
