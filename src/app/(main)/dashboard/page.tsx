  ...
  return (
    <div className={`flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden ${className}`}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-shrink-0">
        <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wide">{title}</h2>
        {viewHref && (
          <Link href={viewHref} className="text-sm font-semibold text-slate-700 hover:text-slate-900 transition-colors">
            View all →
          </Link>
        )}
      </div>
...
function Empty({ message }: { message: string }) {
  return (
    <div className="flex items-center justify-center h-full py-6 text-slate-400 text-sm">{message}</div>
  );
}
    ...
    <div className="h-full flex flex-col overflow-hidden bg-slate-50">
      ...
      <div className="flex-shrink-0 px-6 py-4 border-b border-slate-100 bg-white">
        <h2 className="text-xl font-bold text-slate-900">
          Welcome back, <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-lg">{user?.name ?? 'there'}</span>! 👋
        </h2>
      ...
      {/* ── Content grid ──────────────────────────────────────────────────── */}
      <div className="flex-1 min-h-0 px-5 pt-2 pb-4 flex gap-4 text-[15px]">
                  ...
                  return (
                    <div key={s.status}>
                      <div className="flex items-center justify-between mb-1">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-sm font-semibold ${STATUS_COLORS[s.status] ?? 'bg-slate-100 text-slate-600'}`}>
                          {STATUS_LABELS[s.status] ?? s.status}
                        </span>
                        <div className="flex items-center gap-2">
                          <span className="text-base font-bold text-slate-700 tabular-nums">{s.count}</span>
                          <span className="text-sm text-slate-400 tabular-nums w-10 text-right">{pct}%</span>
                        </div>
                      </div>
              ...
              <ul className="divide-y divide-slate-100">
                {data.recentCalls.map((c) => (
                  <li key={c.id} className="flex items-center gap-4 px-5 py-4">
                    <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center ${c.direction === 'OUTBOUND' ? 'bg-slate-900' : 'bg-slate-100'}`}>
                      <svg className={`w-5 h-5 ${c.direction === 'OUTBOUND' ? 'text-white' : 'text-slate-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                      </svg>
                    </div>
                    <div className="flex-1 min-w-0">
                      <Link href={`/candidates/${c.candidate.id}`} className="text-base font-semibold text-slate-800 hover:text-slate-900 truncate block transition-colors">
                    ...
                    <span className="text-sm text-slate-400 flex-shrink-0 whitespace-nowrap">{formatDateTime(c.createdAt)}</span>
                  </li>
                ))}
              </ul>
            ...
            {isLoading ? <Pulse rows={4} /> : data && data.recentMessages.length > 0 ? (
              <ul className="divide-y divide-slate-100">
                {data.recentMessages.map((m) => (
                  <li key={m.candidateId} className="px-5 py-4">
                    <Link href={`/inbox/${m.candidateId}`} className="flex items-center gap-4 hover:opacity-80 transition-opacity">
                      <div className="flex-shrink-0 w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 text-base font-bold">
                        {m.candidateName.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-base font-semibold text-slate-800 truncate">{m.candidateName}</span>
                          <span className="text-sm text-slate-400 flex-shrink-0">{formatDateTime(m.lastMessageAt)}</span>
                        </div>
                        <p className="text-sm text-slate-500 truncate leading-snug mt-0.5">
            ...
            {isLoading ? <Pulse /> : data && data.recentCandidates.length > 0 ? (
              <ul className="divide-y divide-slate-100">
                {data.recentCandidates.map((c) => (
                  <li key={c.id} className="flex items-center gap-4 px-5 py-4">
                    <div className="flex-shrink-0 w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center text-white text-base font-bold">
                      {c.fullName.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <Link href={`/candidates/${c.id}`} className="text-base font-semibold text-slate-800 hover:text-slate-900 truncate block transition-colors">
                        {c.fullName}
                      </Link>
                      <p className="text-sm text-slate-400 leading-snug mt-0.5">{c.phoneNumber}</p>
                    </div>
                    <span className={`text-sm px-2.5 py-1 rounded-md font-semibold flex-shrink-0 ${STATUS_COLORS[c.status] ?? 'bg-slate-100 text-slate-600'}`}>
