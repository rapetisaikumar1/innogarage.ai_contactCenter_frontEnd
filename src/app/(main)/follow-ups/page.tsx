  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between gap-4 px-8 pt-8 pb-6 border-b border-gray-200 bg-white">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Follow-ups</h1>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              ...
              <p className="text-sm font-medium text-gray-500">No follow-ups found</p>
              <p className="text-xs text-gray-400 mt-1">Try switching to a different filter</p>
            </div>
          ) : (
            <>
              <div className="divide-y divide-gray-100">
                {data.followUps.map((fu) => (
                  <div key={fu.id} className="px-6 py-4 first:pt-5 last:pb-5 hover:bg-gray-50/60 transition-colors">
                    {fu.candidate && (
                      <Link
                        href={`/candidates/${fu.candidate.id}`}
                        className="text-xs font-semibold text-gray-600 hover:text-gray-700 mb-1 inline-block transition-colors"
                      >
              ...
              {pagination && pagination.totalPages > 1 && (
                <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                  <p className="text-sm text-gray-500">
                    Page <span className="font-semibold text-gray-700">{page}</span> of <span className="font-semibold text-gray-700">{pagination.totalPages}</span>
                  </p>
