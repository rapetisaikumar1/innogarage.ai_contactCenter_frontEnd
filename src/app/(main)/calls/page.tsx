...
const STATUS_STYLES: Record<CallStatus, { cls: string; icon: React.ReactNode }> = {
  COMPLETED: {
    cls: 'bg-gray-50 text-gray-700 ring-1 ring-gray-200',
    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-4 px-8 pt-8 pb-6 border-b border-gray-200 bg-white">
        ...
        <button
          onClick={refetch}
          disabled={isLoading}
          className="inline-flex items-center gap-2 px-3 py-2.5 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 transition-colors"
        >
      ...
      <div className="p-8 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          {error ? (
            <div className="flex items-center justify-center py-16 text-sm text-gray-500">{error}</div>
          ) : isLoading ? (
            <div className="divide-y divide-gray-100">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex items-center gap-4 px-6 py-4">
                  ...
                  <div className="h-6 w-24 bg-gray-100 rounded-full animate-pulse ml-4" />
                  <div className="h-6 w-14 bg-gray-100 rounded-lg animate-pulse ml-auto" />
                </div>
              ))}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-500 uppercase tracking-wider">Direction</th>
                    <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
                    <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-500 uppercase tracking-wider">Duration</th>
                    <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-500 uppercase tracking-wider">Logged By</th>
                    <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-500 uppercase tracking-wider">Date &amp; Time</th>
                  ...
                  {data.calls.map((call) => (
                    <tr key={call.id} className="hover:bg-gray-50/80 transition-colors group">
                      <td className="px-6 py-4">
                        <Link href={`/candidates/${call.candidateId}`} className="font-semibold text-gray-800 hover:text-gray-600 transition-colors text-sm">
                          {call.candidate.fullName}
                        </Link>
                        <p className="text-xs text-gray-400 mt-0.5">{call.phoneNumber}</p>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-xs font-mono font-bold text-gray-700 bg-gray-50 ring-1 ring-gray-200 px-2.5 py-1 rounded-lg">
                          {formatDuration(call.duration)}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-600">{call.loggedBy.name}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{formatDateTime(call.createdAt)}</td>
              ...
              {data.pagination.totalPages > 1 && (
                <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                  <p className="text-sm text-gray-500">
                    Page <span className="font-semibold text-gray-700">{data.pagination.page}</span> of <span className="font-semibold text-gray-700">{data.pagination.totalPages}</span>
                  </p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setPage((p) => Math.max(1, p - 1))}
                      disabled={page === 1}
                      className="px-4 py-2 text-sm font-medium border border-gray-200 bg-white rounded-xl hover:bg-gray-50 disabled:opacity-40 transition-colors"
                    >
                      ← Previous
                    </button>
                    <button
