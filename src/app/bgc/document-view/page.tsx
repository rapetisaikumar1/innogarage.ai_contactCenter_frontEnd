    ...
    <main className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto flex min-h-full max-w-7xl flex-col gap-5">
        <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-950">Document Viewer</h1>
            <p className="mt-1 text-sm text-slate-500">{name}</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <button type="button" onClick={() => window.close()} className="rounded-xl border border-black bg-white px-4 py-2 text-sm font-semibold text-black hover:bg-slate-50">
              Close tab
            </button>
